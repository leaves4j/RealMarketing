  ...
  sign_list: function (req, res, next) {
    ...
    async.auto({
      sign_list: function (cb) {
        console.log(1);
        Sign.find({period: 13}).populate('user').sort({'_id': 1}).exec(cb)
      },
      all_sign_count: function (cb) {
        console.log(12);
        Sign.count({period: 13}, cb)
      },
      time_sign_count: function (cb) {
        console.log(14);
        Sign.find({period: 13}).where('timestamp').lte(new Date(require('../config/system.json').start)).count().exec(cb)
      }
